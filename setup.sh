...
PLIST_LABEL="com.wellness.notifier"
PLIST_PATH="$HOME/Library/LaunchAgents/$PLIST_LABEL.plist"
LOG_PATH="$HOME/Library/Logs/wellness_notifier.log"
...
# 3. 检查 notifier.py
if [ ! -f "$NOTIFIER" ]; then
  echo "❌ 未找到 notifier.py，请确保 setup.sh 和 notifier.py 在同一目录"
  exit 1
fi
